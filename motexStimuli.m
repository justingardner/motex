...
%         by: justin gardner
%       date: 06/20/19
%    purpose: Copied from motexWidefieldStimuli - can now also get
%             behavioral images
%
%             Pulls images from Akshay's folders and converts them into
...
if behavior
  dispHeader('Running for behavioral images');
  texFolder = 'tex_12samples';
  noiseFolder = 'noise_12samples';
  texGenTypes = {'pool4'};
  searchFolders = {texFolder};
else
...
if behavior
  ...
  for iFile= 1:length(matchFilenames{1})
    ...
    e.texFamilyNum(iTrial) = find(strcmp(e.texFamily{iTrial},unique(matchTexFamily{1})));
    ...
    for iFolder = 1:length(searchFolders)
      ...
      % rescale image
      sourceImage = motexStimuliRescale(sourceImage,outputSize);
      % keep image
	...
	e.controlFilename{iTrial} = sourceFilename;
      ...
      % create destFilename
      destFilename = sprintf('%s_family_%04i_sample_%04i',destFilenameStem,e.texFamilyNum(iTrial),e.texSampleNum(iTrial));
...
%    motexStimuliRescale    %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function sourceImage = motexStimuliRescale(sourceImage,outputSize)
