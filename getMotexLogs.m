...
if ~isinf(sessionNum)
  if any(sessionNum < 1) | any(sessionNum > d.nSessions)
    dispHeader(sprintf('(getMotexLogs) Session %s does not exist',mlrnum2str(sessionNum,'sigfigs=0')));
    return
  end
else
...
if ~isfield(p,'Protocol')
  disp(sprintf('(getMotexLogs:motexGetStimulusInfo) No Protocol variable in %s',runInfo.protocolPath));
  return
else
...
if fPFile == 0
  disp(sprintf('(getMotexLogs:motexGetStimulusInfo) Could not open %s',runInfo.pFilePath));
else
  % read the header
...
if ~isfile(runInfo.logPath)
  disp(sprintf('(getMotexLogs:motexSyncToAI) Could not find log %s',runInfo.logPath));
  return
end

% load the log
disppercent(-inf,sprintf('(getMotexLogs:motexSyncToAI) Loading log %s',runInfo.logPath));
log = load(runInfo.logPath);
disppercent(inf);
if ~isfield(log,'AIdata')
  disp(sprintf('(getMotexLogs:motexSyncToAI) Log file %s does not contain AIdata',runInfo.logPath));
  return
end
...
disp(sprintf('(getMotexLogs:syncToAI) %s Session %i Run %i: %i Camera trials (%s images per trial) %0.2f Hz',d.dataDir,iSession,iRun,runInfo.cameraOnN,mlrnum2str(unique(runInfo.acqPerCameraOn),'sigfigs=0'),runInfo.cameraFreq));
  
% check to make sure photodiode trials match camera triggers
if runInfo.cameraOnN ~= runInfo.photoDiodeTimes.nTrials
  disp(sprintf('(getMotexLogs:syncToAI) Camera on triggers do not match photoDiode trials: %i ~= %i',runInfo.cameraOnN,runInfo.photoDiodeTimes.nTrials));
...
if maxPhotoDiode < 1
  disp(sprintf('(getMotexLogs:motexGetPhotoDiodeTimes) The photoDiode trace does not have any values above 1'));
  return
end
...
if length(trigsPerTrial) > 1
  disp(sprintf('(getMotexLogs:motexGetPhotoDiodeTimes) %i trials do not have all the same length of photo triggers %s',nTrials,mlrnum2str(trigsPerTrial)));
end
...
% display
hFig = mlrSmartfig('getMotexLogs','reuse');clf;
subplot(3,1,1);
plot(t,photoDiode,'k-');
hold on
