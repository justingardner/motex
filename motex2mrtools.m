...
if ~isdir(d.dataFullPath)
  disp(sprintf('(motex2mrtools) Could not find data directory: %s',dataDir));
  return
end
...
d.nSessions = d.nSessions-1;
disp(sprintf('(motex2mrtools) Found %i sessions in %s',d.nSessions,d.dataDir));
...
for iSession = 1:d.nSessions
  ...
  for iRun = 1:d.nRuns(iSession)
    ...
    if ~isfile(d.runInfo{iSession}{iRun}.logPath)
      disp(sprintf('(motex2mrtools) Could not find VS_LOGS file: %s',d.runInfo{iSession}{iRun}.logPath));
    ...
    if ~isfile(d.runInfo{iSession}{iRun}.screenInfoPath)
      disp(sprintf('(motex2mrtools) Could not find screenInfo: %s',d.runInfo{iSession}{iRun}.screenInfoPath));
    else
      warning off
    ...
    if ~isfile(d.runInfo{iSession}{iRun}.protocolPath)
      disp(sprintf('(motex2mrtools) Could not find protocol: %s',d.runInfo{iSession}{iRun}.protocolPath));
    ...
    if ~isfile(d.runInfo{iSession}{iRun}.pFilePath)
      disp(sprintf('(motex2mrtools) Could not find p file: %s',d.runInfo{iSession}{iRun}.pFilePath));
      d.runInfo{iSession}{iRun}.pFilePath = '';
    end
  end
end
disp(sprintf('(motex2mrtools) Found %s runs in sessions',mlrnum2str(d.nRuns,'sigfigs=0')));
