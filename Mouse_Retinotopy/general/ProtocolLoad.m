...
% 2009-02 AZ (UNDONE) Append '\Cat' or '\Mouse' to DIRS.MPEP, based on first letter of animal
...
ProtocolFileName = fullfile(DIRS.MPEP,animal,int2str(iseries),int2str(iexp), 'Protocol.mat');
...
%         end
    ...
    fullpfilename1 = fullfile(DIRS.MPEP,animal,int2str(iseries),int2str(iexp),pfilename);
    fullpfilename2 = fullfile(DIRS.MPEP,animal,int2str(iseries),pfilename);
...
%         protocol.pfilefreqs = (protocol.pars(14,:)~=0).*(protocol.pars(15,:)); % frequency of optical stimulus
    ...
    case {'stimMarshel.x'} %added by Mohammad 20150917
        protocol.blankpars = [11 12 13]; 
        protocol.pfiledurs = protocol.pars(1,:)/10; 
...
if isstr(screenflag) & ((strcmpi(animal,'catz') & str2num(animal(end-2:end)) > 12) | ~strcmp(lower(animal),'catz')) ...
    ...
    if ~isempty(nif) & ~isempty(protocol.pfilefreqs)
        [motherdir,datadir] = fileparts(DIRS.MPEP);
